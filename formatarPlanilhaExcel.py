...
from caminhos import caminhoPlanilhaNotasDebito,caminho_planilha_notas_CAP_aberto,planilhaPastaNotasCanceladas,caminho_planilha_notas_CAP,CAMINHO_DESTINO, CAMINHO_NOTAS_DEBITOS,CAMINHO_NOTAS_CREDITOS,CAMINHO_NOTAS_PAGAMENTOS,CAMINHO_NOTAS_CANCELADAS
...
def limpar_boletos_e_excluir_cabeçalhos(empresa):
    ...
    if 'lokan' == empresa:
        arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokan.xlsx'  # Arquivo onde as abas serão criadas

    else:
        arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokanfacil.xlsx'
    ...
    try:
        ...
        if 'lokan' == empresa:
            ...
            arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokan.xlsx'  # Arquivo onde as abas serão criadas
             ...
             # Caminhos dos arquivo 
            arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokanfacil.xlsx'  # Arquivo onde as abas serão criadas
...
def verifica_pgs_notas_creditos(empresa):
    ...
    try:
        ...
        # Carregar todas as abas da planilha principal
        planilha_principal = pd.ExcelFile(arquivo_destino)
        abas_principais = planilha_principal.sheet_names
        ...
        # Carregar a planilha secundária
        ...
        # Verifique se as colunas existem na planilha secundária
        for col in [coluna_documento, coluna_liquidado, coluna_valor]:
            if col not in planilha_secundaria.columns:
                infoLogs().info(f"A coluna '{col}' não existe na planilha secundária.")
                return

        # Função para limpar o valor, removendo espaços e caracteres especiais
        def limpar_valor(valor):
            if isinstance(valor, str):
                return valor.strip().replace('\n', '').replace('\r', '')
            else:
        ...
        # Remover duplicatas e criar dicionário para mapear dados
        planilha_secundaria = planilha_secundaria.drop_duplicates(subset=coluna_documento, keep='first')
        dados_secundarios = planilha_secundaria.set_index(coluna_documento).to_dict(orient='index')

        def atualizar_colunas(row):
            # Ignorar linhas onde a coluna PGs já possui os valores 'PG' ou 'BX'
            if row[coluna_pg] in ['PG', 'BX']:
                return row[coluna_pg], row[coluna_valor], row[coluna_recbo]

            # Ignorar linhas que já possuem valores na coluna CREDITO
            if not pd.isna(row[coluna_credito]) and row[coluna_credito] != '':
                return row[coluna_pg], row[coluna_valor], row[coluna_recbo]

            # Realizar verificação apenas se houver valor na coluna DEBITO
            if pd.isna(row[coluna_valor]) or row[coluna_valor] == '':
                return row[coluna_pg], row[coluna_valor], row[coluna_recbo]

            nfiscal_principal = limpar_valor(str(row[coluna_documento]))  # Converter para string e limpar

            # Verifica se o NFISCAL está na planilha secundária
            if nfiscal_principal in dados_secundarios:
                valor_value = dados_secundarios[nfiscal_principal].get(coluna_valor, row[coluna_valor])
                recbo_value = dados_secundarios[nfiscal_principal].get(coluna_liquidado, row[coluna_recbo])
                return 'PG', valor_value, recbo_value  # Atualiza para 'PG', junto com os outros valores
            else:
                return 'AB', row[coluna_valor], row[coluna_recbo]  # Define como 'AB' se não encontrado

        # Iterar por todas as abas da planilha principal
        with pd.ExcelWriter(arquivo_destino, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            for aba in abas_principais:
                # Carregar os dados da aba atual
                planilha_atual = pd.read_excel(arquivo_destino, sheet_name=aba)

                # Verificar se as colunas necessárias estão na aba atual
                if all(col in planilha_atual.columns for col in [coluna_documento, coluna_pg, coluna_valor, coluna_recbo, coluna_credito]):
                    # Aplicar a função para atualizar as colunas
                    planilha_atual[[coluna_pg, coluna_valor, coluna_recbo]] = planilha_atual.apply(
                        atualizar_colunas, axis=1, result_type='expand'
                    )

                # Salvar a aba atualizada
                planilha_atual.to_excel(writer, sheet_name=aba, index=False)
...
def formatar_planilha_notas_credito(empresa):
    ...
    try:
        ...
        df = pd.read_excel(f'{caminho}\\{nome_xls}', engine='xlrd', header=11, usecols=colunas)
...
def formatarPlanilhaPagamentos(empresa):
    ...
    try:
        ...
        df = df[['Unnamed: 1','Unnamed: 6', 'Unnamed: 9', 'Unnamed: 10', 'Unnamed: 13', 'Unnamed: 27']]
...
def remover_duplicados_chave_unica(colunas_chave,empresa):
    ...
    Args:
        ...
        colunas_chave (list): Lista das letras das colunas que compõem a chave única.
    """
    if 'lokan' == empresa:
    ...
    for aba in wb.sheetnames:  # Itera por todas as abas
        ...
        for linha in range(2, ws.max_row + 1):
            # Constrói a chave única concatenando os valores das colunas especificadas
            valores_chave = tuple(ws[f'{coluna}{linha}'].value for coluna in colunas_chave)
            
            # Verifica se todas as colunas da chave possuem valores
            if all(valores_chave):
                if valores_chave in chaves_unicas:
                    linhas_para_remover.append(linha)  # Marca a linha para remoção se a chave for duplicada
                else:
                    chaves_unicas.add(valores_chave)
...
def criar_planilha_principal(empresa):
    ...
    if 'lokan' == empresa:
        # Caminho do arquivo
        arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokan.xlsx'
    else:
        # Caminho do arquivo
        arquivo_destino = rf'{CAMINHO_DESTINO}\planilha_principal_lokanfacil.xlsx'
...
def excluir_aba(empresa):
    try:
        ...
        if 'lokan' == empresa:
            caminho_arquivo = rf'{CAMINHO_DESTINO}\planilha_principal_{empresa}.xlsx'  # Caminho da planilha
        else:
            caminho_arquivo = rf'{CAMINHO_DESTINO}\planilha_principal_{empresa}.xlsx'  # Caminho da planilha
...
def formatacao_planilha_final():
    ...
    for i in EMPRESAS:
        ...
        # Uso da função
        colunas_chave=['E', 'F', 'A']
        remover_duplicados_chave_unica(colunas_chave,empresa)  # Colunas: NFISCAL (E), NOME (F), VENCTO (A))
